...
| P3 | Organization & control | DONE | CI green 2026-09-20 (5 slices). P3a categories/auto-sort lite: Room v3, Categorizer ext/MIME/host (tested), per-category folders, filter chips, sheet picker w/ auto badge, Settings add/delete. P3b: DuplicateTask dialog (tested), batch paste-a-list, URL export, FAB paste-and-start, SEND/VIEW/PROCESS_TEXT intents. P3c: multi-select toolbar, swipe right action/left delete-Undo, rename/move/share-file, SHA-256 verify (tested). P3d: GatePolicy wifi/charging/schedule (tested) + GateMonitor + waiting banner, global speed cap (tested). P3e: vault (.nomedia, Vault filter) + app lock (credential on start/resume), SETTINGS.md filled. Checklists: waiting vocabulary ✓, RTL auto-mirror swipes ✓, selection = tint+count ✓, strings in res ✓. PARTIAL gaps: named queues, per-task limits, battery-%, tags, vault-view gate, sort preview+undo, batch per-item errors, sticky header (→ P6); i18n → P7 |
